...
@section('content')
    <main class="workspace">
        ...
        @if($errors->any())
            ...
            @endforeach
        @endif
        <form id="schoolForm" action="{{ route('admin.schools.updateStore', ['school' => $school]) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="lg:col-span-2 xl:col-span-3">
                    <div class="card p-5">
                        <div class="mb-5">
                            ...
                            <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $school->name) }}">
                        </div>
                        <div class="mb-5">
                            <label class="label block mb-2" for="email_pattern">Email Paterni</label>
                            <input type="text" id="email_pattern" name="email_pattern" class="form-control" value="{{ old('email_pattern', $school->email_pattern) }}">
                        </div>
                        <div class="mb-5">
                            <label class="label block mb-2" for="latitude">Enlem</label>
                            <input type="text" id="latitude" name="latitude" class="form-control" value="{{ old('latitude', $school->latitude) }}">
                        </div>
                        <div class="mb-5">
                            <label class="label block mb-2" for="longitude">Boylam</label>
                            <input type="text" id="longitude" name="longitude" class="form-control" value="{{ old('longitude', $school->longitude) }}">
                        </div>
                        <div class="mb-5">
                            <label class="label block mb-2" for="latitude_delta">Enlem Delta</label>
                            <input type="text" id="latitude_delta" name="latitude_delta" class="form-control" value="{{ old('latitude_delta', $school->latitude_delta) }}">
                        </div>
                        <div class="mb-5">
                            <label class="label block mb-2" for="longitude_delta">Boylam Delta</label>
                            <input type="text" id="longitude_delta" name="longitude_delta" class="form-control" value="{{ old('longitude_delta', $school->longitude_delta) }}">
                        </div>
                    </div>
                </div>
                        ...
                        <div class="flex flex-col gap-y-5">
                            <div class="flex items-center">
                                ...
                                <div class="w-1/4 ml-2">
                                    <label class="switch">
                                        <input type="checkbox" name="is_active" @if($school->is_active) checked @endif>
                                        <span></span>
                                    </label>
                                </div>
                            </div>
