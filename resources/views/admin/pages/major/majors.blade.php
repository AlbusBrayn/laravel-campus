        ...
        <section class="breadcrumb lg:flex items-start">
            ...
            <div class="flex flex-wrap gap-2 items-center ltr:ml-auto rtl:mr-auto mt-5 lg:mt-0">
                ...
                <div class="flex gap-x-2">
                    <a href="{{ route('admin.majors.create') }}" class="btn btn_primary uppercase">Yeni Ekle</a>
                </div>
            </div>
        </section>
        <div class="card p-5">
            <div class="overflow-x-auto">
                <table class="table table-auto table_hoverable w-full">
                    <thead>
                    <tr>
                        <th class="text-center uppercase">Bölüm Adı</th>
                    ...
                    @foreach($majors as $major)
                        <tr>
                            <td class="text-center uppercase">{{ $major->title }}</td>
                            ...
                            <td class="text-center whitespace-nowrap">
                                <div class="inline-flex ltr:ml-auto rtl:mr-auto">
                                    <a href="{{ route('admin.majors.update', ['major' => $major]) }}" class="btn btn-icon btn_outlined btn_secondary">
                                        <span class="la la-pen-fancy"></span>
                                    </a>
                                    <form method="POST" onsubmit="return confirm('Bu bölümü silmek istediğinden emin misin?');" action="{{ route('admin.majors.delete', ['major' => $major]) }}">
                                        @csrf
                                        @method('DELETE')
