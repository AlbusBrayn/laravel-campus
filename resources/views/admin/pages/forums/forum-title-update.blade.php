...
@section('content')
    <main class="workspace">
        <section class="breadcrumb lg:flex items-start">
            <div>
                <h1>Forum Başlığı Güncelle</h1>
                <ul>
                    <li><a class="{{ route('admin.dashboard') }}">Annasayfa</a></li>
                    <li class="divider la la-arrow-right"></li>
                    <li><a href="{{ route('admin.forums.titles') }}">Forum Başlığı Listesi</a></li>
                    <li class="divider la la-arrow-right"></li>
                    <li><a>Forum Başlığı Güncelle</a></li>
                </ul>
            </div>
            <div class="flex flex-wrap gap-2 items-center ltr:ml-auto rtl:mr-auto mt-5 lg:mt-0">
                <div class="flex gap-x-2">
                    <button onclick="document.getElementById('forumTitleForm').submit()" class="btn btn_primary uppercase">Kaydet</button>
                </div>
            </div>
        </section>
        @if($errors->any())
            ...
            @endforeach
        @endif
        <form id="forumTitleForm" action="{{ route('admin.forums.titles.updateStore', ['postTitle' => $postTitle]) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="lg:col-span-2 xl:col-span-3">
                    <div class="card p-5">
                        <div class="mb-5">
                            ...
                            <input type="text" id="title" name="title" class="form-control" value="{{ old('title', $postTitle->translate('tr')->title) }}">
                        </div>
                    </div>
                </div>
                <div class="flex flex-col gap-y-5 lg:col-span-2 xl:col-span-1">
                    <div class="card p-5 flex flex-col gap-y-5">
                        <h3>Forum Başlığı Ayarları</h3>
                        <div class="flex flex-col gap-y-5">
                            <div class="flex items-center">
                                ...
                                <div class="w-1/4 ml-2">
                                    <label class="switch">
                                        <input type="checkbox" name="is_active" @if($postTitle->is_active) checked @endif>
                                        <span></span>
                                    </label>
                                </div>
                            </div>
